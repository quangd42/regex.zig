...
pub fn match(vm: *Vm, haystack: []const u8) bool {
    return vm.search(.none, haystack) != null;
}

pub fn find(vm: *Vm, haystack: []const u8) ?Match {
    const slots = vm.search(.bounds, haystack) orelse return null;
    return buildMatch(slots);
}

pub fn findCaptures(vm: *Vm, haystack: []const u8, buffer: []?Match) ?Captures {
    const slots = vm.search(.full, haystack) orelse return null;
...
fn search(vm: *Vm, comptime mode: Mode, haystack: []const u8) ?[]const Offset {
    vm.current_states.clear();
    vm.next_states.clear();

    const start = vm.literalPrefixOffset(haystack) orelse return null;
...
fn epsilonClosure(
    ...
    while (!vm.stack.isEmpty()) {
        ...
        switch (top) {
            ...
            .explore => |e| vm.explore(mode, e, at, slots),
        }
    }
}
...
fn step(vm: *Vm, comptime mode: Mode, target: u8, at: Offset) ?[]const Offset {
    vm.current_states.clear();
    ...
    for (vm.current_states.slice()) |id| {
        ...
        switch (vm.prog.states[id]) {
            .char => |s| if (target == s.byte) {
                vm.epsilonClosure(mode, s.out, at + 1, slots);
            },
            .ranges => |s| {
...
fn seedStartState(vm: *Vm, comptime mode: Mode, at: Offset) void {
